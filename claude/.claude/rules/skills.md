...
| **one-off** | `/one-off` or "one-off task" or "quick task" | Single bounded task: 2-3 clarifying questions → specialist dispatch → TDD impl → commit. No kanban, no PRD. |
| **compound** | After receiving structured code review feedback that contains patterns or rules worth capturing permanently | Extract lessons from code review and write as permanent rules into CLAUDE.md |
