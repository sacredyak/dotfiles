...
local function handle_backslash_escape(sm)
  ...
  if sm.match then
    if (sm.match == "u" and #sm.ext == 4) or (sm.match == "U" and #sm.ext == 8) then
...
local function remove_underscores_number(sm, number, anchor)
  if number:find("_") then
    if number:find("__") then
      _error(sm, "Numbers cannot have consecutive underscores. Found " .. anchor .. ": " .. number, anchor)
    end
    ...
    if number:find("%D_%d") or number:find("%d_%D") then
      _error(sm, "Underscores must have digits on either side. Found " .. anchor .. " : " .. number, anchor)
    end
...
local function validate_float(sm, value)
  ...
  if sm.match then
    if sm.match:find("_[eE]") or sm.match:find("[eE]_") then
      _error(sm, "Underscores in floats cannot be before or after the e. Found float: " .. sm.match .. sm.ext, "float")
    end
    sm.match = remove_underscores_number(sm, sm.match, "float")
    sm.value = tonumber(sm.match)
    sm.value_type = "float"
...
local function validate_datetime(sm, value)
  ...
  if sm.match then
    if _tointeger(hour) > 23 then
      _error(sm, "Hours must be less than 24. Found hour: " .. hour .. "in: " .. sm.match, "local-time")
    end
    if _tointeger(min) > 59 then
      _error(sm, "Minutes must be less than 60. Found minute: " .. min .. "in: " .. sm.match, "local-time")
    end
    if _tointeger(sec) > 60 then
      _error(sm, "Seconds must be less than 61. Found second: " .. sec .. "in: " .. sm.match, "local-time")
    end
    if sm.ext ~= "" then
  ...
  if sm.match then
    ...
    if month == 0 or month > 12 then
      _error(sm, "Month must be between 01-12. Found month: " .. month .. "in: " .. sm.match, "local-date")
    end
        ...
        if day > 28 then
          _error(
            ...
            "Too many days in month. Found "
              .. day
              .. " days in month 02, which only has 28 days if it's not a leap year in: "
              .. sm.match,
            "local-date"
          )
        end
  ...
  if sm.match then
    if _tointeger(hour) > 23 then
      _error(sm, "Hours must be less than 24. Found hour: " .. hour .. "in: " .. sm.match, "")
    end
    if _tointeger(min) > 59 then
      _error(sm, "Minutes must be less than 60. Found minute: " .. min .. "in: " .. sm.match)
    end
    if _tointeger(sec) > 60 then
      _error(sm, "Seconds must be less than 61. Found second: " .. sec .. "in: " .. sm.match)
    end
    year, month, day = _tointeger(year_s), _tointeger(month_s), _tointeger(day_s)
    if month == 0 or month > 12 then
      _error(sm, "Month must be between 01-12. Found month: " .. month .. "in: " .. sm.match)
    end
    if day == 0 or day > max_days_in_month[month] then
      _error(
        sm,
      ...
      if leap_year == false then
        if day > 28 then
          _error(
            ...
            "Too many days in month. Found "
              .. day
              .. " days in month 02, which only has 28 day if it's not a leap year in: "
              .. sm.match,
            "local-datetime"
          )
        end
    ...
    if sm.ext ~= "" then
      ...
      elseif sm.ext:find("^%.%d+[+-]%d%d:%d%d$") then
        ...
        if _tointeger(hour) > 23 then
          _error(sm, "Hours must be less than 24. Found hour: " .. hour .. "in: " .. sm.match, "offset-date-time")
        end
        if _tointeger(min) > 59 then
          _error(sm, "Minutes must be less than 60. Found minute: " .. min .. "in: " .. sm.match, "offset-date-time")
        end
        sm.value_type = "datetime"
        sm.value = sm.type_conversion[sm.value_type](sm.match .. sm.ext:sub(1, 4))
        return true
...
local function generic_assign(value)
  return value
end
